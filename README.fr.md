...
C'est tout. Votre serveur est prêt. Un nom à vous, votre propre certificat TLS, ou un serveur derrière du NAT : [options d'installation](docs/02-installation/install.md).
...
Vous préférez le faire depuis le serveur ? [Déployer directement depuis git](docs/README.md#3-put-a-site-online).
...
- **Cloudflare en une connexion.** DNS, tunnels et cache pour vos projets, y compris un site sur un serveur derrière du NAT. [Domaines et HTTPS](docs/05-capabilities/domains-and-ssl.md)
...
Toute la procédure : [quand un déploiement échoue](docs/09-deploy-failures/what-happens.md) · [ce que veut dire l'erreur](docs/09-deploy-failures/reading-errors.md).
...
Ce qui est collecté, ce qui ne l'est pas, et toutes les façons de le couper : [ce qui est collecté](docs/08-telemetry/what-is-collected.md) · [comment le couper](docs/08-telemetry/how-to-turn-it-off.md).
...
Oui. PanelAlpha Engine est un outil qui permet aux agents IA de gérer *votre* serveur. Il vous faut un VPS Debian ou Ubuntu neuf, avec au moins 2 Go de RAM et 1 CPU, sur lequel vous vous connectez en `root` par SSH. Vous commencez en lançant vous-même l'installation d'Engine sur ce VPS. [Ce qu'il vous faut](docs/02-installation/install.md#before-you-start)
</details>
