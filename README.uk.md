...
Це все. Сервер готовий. Власна назва, власний сертифікат TLS чи сервер за NAT: [опції встановлення](docs/02-installation/install.md).
...
Волієте зробити це із сервера самі? [Розгортання прямо з git](docs/README.md#3-put-a-site-online).
...
- **Просте підключення Cloudflare.** DNS, тунелі й кешування в усіх своїх проєктах, зокрема для сайту на сервері за NAT. [Домени і HTTPS](docs/05-capabilities/domains-and-ssl.md)
...
Уся процедура: [коли розгортання не вдається](docs/09-deploy-failures/what-happens.md) · [що означає помилка](docs/09-deploy-failures/reading-errors.md).
...
Що збирається, що ні, і всі способи вимкнути: [що збирається](docs/08-telemetry/what-is-collected.md) · [як це вимкнути](docs/08-telemetry/how-to-turn-it-off.md).
...
Так. PanelAlpha Engine є інструментом, за допомогою якого ШІ-агенти керують *вашим* сервером. Потрібен чистий VPS із Debian або Ubuntu, щонайменше 2 ГБ RAM і 1 CPU, на який ви заходите як `root` через SSH. Ви починаєте з того, що самі запускаєте встановлення Engine на цьому VPS. [Що потрібно](docs/02-installation/install.md#before-you-start)
</details>
