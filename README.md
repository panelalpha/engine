...
That is it. Your server is ready. Custom name, your own TLS certificate, or a server behind NAT: [Install options](docs/02-installation/install.md).
...
Prefer to do it from the server yourself? [Deploy straight from git](docs/README.md#3-put-a-site-online).
...
- **Cloudflare in one connection.** DNS, tunnels and caching across your projects, including a site on a server behind NAT. [Domains and HTTPS](docs/05-capabilities/domains-and-ssl.md)
...
The whole procedure: [when a deploy fails](docs/09-deploy-failures/what-happens.md) · [what the error means](docs/09-deploy-failures/reading-errors.md).
...
What is collected, what is not, and every way to turn it off: [what is collected](docs/08-telemetry/what-is-collected.md) · [how to turn it off](docs/08-telemetry/how-to-turn-it-off.md).
...
An anonymous summary: the kind of application, the stage that broke, a redacted log tail, and the public names of the sites. Never your source, project names, credentials or your server's identity. You can print a queued report with `pae telemetry:show`, and turn the whole thing off. Full detail: [Telemetry](docs/08-telemetry/what-is-collected.md).
</details>
