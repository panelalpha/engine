...
就这样，服务器已经就绪。自定义名称、自备 TLS 证书，或者服务器在 NAT 后面：[安装选项](docs/02-installation/install.md)。
...
更想自己在服务器上做？[直接从 git 部署](docs/README.md#3-put-a-site-online)。
...
- **一次连接即可使用 Cloudflare。** 在所有项目中使用 DNS、隧道和缓存，也能让 NAT 后面的服务器对外提供站点。[域名与 HTTPS](docs/05-capabilities/domains-and-ssl.md)
...
完整流程：[部署失败时](docs/09-deploy-failures/what-happens.md) · [这个报错是什么意思](docs/09-deploy-failures/reading-errors.md)。
...
收集了什么、没收集什么，以及所有关闭方式：[收集了什么](docs/08-telemetry/what-is-collected.md) · [如何关闭](docs/08-telemetry/how-to-turn-it-off.md)。
...
需要。PanelAlpha Engine 是让 AI 智能体管理*你的*服务器的工具。你需要一台全新的 Debian 或 Ubuntu VPS，至少 2 GB 内存和 1 核 CPU，并以 `root` 通过 SSH 登录。你自己先在那台 VPS 上运行 Engine 安装。[你需要准备什么](docs/02-installation/install.md#before-you-start)
</details>
...
一份匿名摘要：应用类型、出错的阶段、脱敏后的日志末尾，以及站点的公开名称。绝不包括源代码、项目名、凭据或服务器身份。你可以用 `pae telemetry:show` 打印队列里的报告，也可以把整套遥测关掉。完整细节：[遥测](docs/08-telemetry/what-is-collected.md)。
</details>
