...
|
| The hub is an integration the engine *calls*: WithoutDNS names, licensing,
...
| addressed to the hub, which does not serve the ingest and answers 405.
| Anything new that reports rather than asks belongs on monitoring, not here.
|
| Empty is a supported answer meaning "no hub": App\Integrations\Tunnels\
| PanelAlphaHub builds no URL from it, and each caller is expected to say so
| rather than calling a relative path.
|
| There is one hub and it is `connect.panelalpha.com`. The variable exists so an
| install can be pointed at a replacement, not so a second environment can be
| kept alongside it: an engine talking to a staging hub allocates real names
...
return [
    ...
    'url' => (string) env('PANELALPHA_HUB', 'http://staging.connect.panelalpha.com/'),
