...
# VITE_UMAMI_* analytics, VITE_STORAGE / VITE_S3_PUBLIC_LINK for Digidrive,
# VITE_UPLOAD_LIMIT — and inc/env.php read()s the copy the bundle carries,
# where SQLite stays the default DB_DRIVER, so an empty file is a valid
# default: everything stays off until the operator fills it in.
if [ ! -f .env.production ]; then
    printf '# Created by PanelAlpha: the Vite build fails without this file.\n# Fill in VITE_UMAMI_SCRIPT_URL / VITE_UMAMI_WEBSITE_ID / VITE_STORAGE / VITE_S3_PUBLIC_LINK / VITE_UPLOAD_LIMIT as needed; DB_DRIVER defaults to SQLite with no server to run.\n' > .env.production
fi