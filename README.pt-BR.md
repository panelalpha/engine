...
É isso. Seu servidor está pronto. Um nome próprio, seu próprio certificado TLS ou um servidor atrás de NAT: [opções de instalação](docs/02-installation/install.md).
...
Prefere fazer pelo servidor? [Publicar direto do git](docs/README.md#3-put-a-site-online).
...
- **Cloudflare numa só conexão.** DNS, túneis e cache em todos os seus projetos, inclusive para servir um site de um servidor atrás de NAT. [Domínios e HTTPS](docs/05-capabilities/domains-and-ssl.md)
...
O procedimento inteiro: [quando um deploy falha](docs/09-deploy-failures/what-happens.md) · [o que o erro quer dizer](docs/09-deploy-failures/reading-errors.md).
...
O que é coletado, o que não é, e todas as formas de desligar: [o que é coletado](docs/08-telemetry/what-is-collected.md) · [como desligar](docs/08-telemetry/how-to-turn-it-off.md).
